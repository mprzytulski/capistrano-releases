...
namespace :versionify do
  ...
  task :prepare, :version do |task, args|
    ...
    if version
      ...
      puts "Created version: #{version.name}"
  ...
  task :active_version do
    ...
    if version
      ...
      puts "Project: #{manager.project.name} [#{manager.project.key}]"
      puts "Active version: #{version.name}"
      puts "URL: #{version_url}"
    else
...
namespace :load do
  task :defaults do
    ...
    set :versionify_podio_api_username, settings['podio']['username']
    set :versionify_podio_api_password, settings['podio']['password']
